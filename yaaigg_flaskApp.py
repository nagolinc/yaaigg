...
from generationFunction import (
    getRandomItem,
    generateNPC,
    generateBuilding,
    generateQuest,
    ...
    while True:
        for destination_queue, generation_function in zip(queues, generators):
            ...
            if not destination_queue.full():
                ...
                # print("\n\n\nENTERING LOCK ZONE\n\n\n")
                item = generator_single(
                    generation_function, star_rating, cost_multiplier
                )
...
def sell_building():
    ...
    user_data["buildings"] = [
        building for building in user_data["buildings"] if building["id"] != id
    ]
...
def get_level_data():
    ...
    return jsonify(
        {"level": level, "experience": exp, "experienceNextLevel": expNextLevel}
    )
...
def predictQuestOutcome(quest, npcs):
    ...
    quest_challenge = (
        quest_star_rating
        * BASE_QUEST_CHALLENGE_MULTIPLIER
        * levelMultiplier(quest_level)
    )
...
def predict_quest_outcome():
    ...
    return jsonify({"p_success": p_success, "timeout": timeout})
...
def custom_static(filename):
    response = make_response(
        send_from_directory(os.path.join(app.root_path, "static/samples"), filename)
    )
        ...
        for building in user_data["buildings"]:
            if building["id"] == npc["location"]:
                ...
                power_level = power_level * (
                    1 + bonus[building_star_rating - 1] * adjustment_factor
                )
                break
...
# Calculate the score based on materials
def calculate_score(materials):
    score = 0
    for material in materials:
        level_modifier = levelMultiplier(material.get("level", 1))
        star_rating = material["star_rating"]
        score += level_modifier * star_rating
    return score


# /upgrade_success_chance endpoint
@app.route("/upgrade_success_chance", methods=["POST"])
def upgrade_success_chance():

    _building_to_upgrade = request.json["building"]
    _materials = request.json["materials"]
    
    #these are passed as id's, we need to find the actual objects in user_data
    building_to_upgrade = None
    for building in user_data["buildings"]:
        if building["id"] == _building_to_upgrade:
    ...
    #if building is none, fail
    if building_to_upgrade is None:
        return jsonify(
            {"probability": 0.0, "message": "Building not found"}
        )
    

    # check if building level is already == player level (can't upgrade)
    if user_data.get("level", 1) <= building_to_upgrade.get("level", 1):
        return jsonify(
            {"probability": 0.0, "message": f"Building level is already at player level"}
        )

    score = calculate_score(materials)
    #required_score = levelMultiplier(
    #    building_to_upgrade.get("new_level", 1) + 1
    #) * building_to_upgrade.get("star_rating", 1)
    
    #this is intentionally wrong, to make the ugrade easier
    required_score = levelMultiplier(
        building_to_upgrade.get("new_level", 1) + 1
    )

    probability = custom_sigmoid(score / required_score)

    # convert to percentage
    s_probability = f"{int(probability*100)}%"

    message = f"building <b>{building_to_upgrade['name']}</b> has a probability of <b>{s_probability}</b> to upgrade"
    
    timeout = 5 + 25 * (1 - probability)

    return jsonify({"probability": probability, "message": message, "timeout": timeout})


# /do_upgrade endpoint
@app.route("/do_upgrade", methods=["POST"])
def do_upgrade():
    _building_to_upgrade = request.json["building"]
    _materials = request.json["materials"]
    ...
    if user_data.get("level", 1) <= building_to_upgrade.get("level", 1):
        return jsonify(
            {
                "status": "failure",
                "message": f"Building <b>{building_to_upgrade.get('name')}</b> is already at player level",
            }
        )
    score = calculate_score(materials)
    ...
    r=random.random()
    print("ROLLING DICE, r=", r, " probability=", probability, r < probability)

    if r < probability:
        building_to_upgrade["level"] += 1
        status = "success"
        message = f"Building <b>{building_to_upgrade['name']}</b> upgraded to level <b>{building_to_upgrade['level']}</b>"
    else:
        status = "failure"
        message = f"Building <b>{building_to_upgrade['name']}</b> upgrade failed"
        
        
    #remove materials from user_data
    user_data["items"] = [item for item in user_data["items"] if item["id"] not in _materials]
    

    saveUserData()

    return jsonify({"status": status, "probability": probability, "message": message})


#/building_management endpoint
@app.route("/building_management")
def building_management():
    return render_template("building_management.html")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--saveData", default="user_data.json", help="Save user data to this file"
    )
    ...
    parser.add_argument(
        ...
        default=4,
        type=int,
        help="Number of threads to start for each generator",
    )

    # --queueSize (default = 4)
    parser.add_argument(
        "--queueSize", default=4, type=int, help="Size of the queue for each generator"
    )

    # --keyword_mapping (default keyword_mapping.yaml)
    parser.add_argument(
        "--keywordMapping", default="keyword_mapping.yaml", help="Keyword mapping file"
    )

    args = parser.parse_args()

    setupKeywordMapping(args.keywordMapping)

    loadUserData(args.saveData)

    item_queue = queue.Queue(
        maxsize=args.queueSize
    )  # Adjust size based on expected load
    npc_queue = queue.Queue(
        maxsize=args.queueSize
    )  # Adjust size based on expected load
    building_queue = queue.Queue(
        maxsize=args.queueSize
    )  # Adjust size based on expected load
