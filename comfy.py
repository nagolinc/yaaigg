...
def generate_comfy(input_prompt,seed=-1,prefix="",yaml_file="comfy.yaml",width=1024,height=1024,suffix=".png",image=None):
    
    
    print("comfyUI is generating>>",input_prompt,"<<with",yaml_file)
    ...
    #static/samples may not exist
    os.makedirs("static/samples", exist_ok=True)
    
    #unique_prefix = find_unique_prefix(api_dir, 'img')
    unique_prefix="img"+generate_random_64bit_string()
    
    #can we add some kind of sanity checking here to make srue this actually exists
    
    
    n=random.randint(0,1000000)
    
    # Ensure the nested keys exist before setting the value
    if all(k in d for k, d in zip(yaml_data['output'], [prompt_data, prompt_data.get(yaml_data['output'][0], {}), prompt_data.get(yaml_data['output'][0], {}).get(yaml_data['output'][1], {})])):
        prompt_data[yaml_data['output'][0]][yaml_data['output'][1]][yaml_data['output'][2]] = f"api/{unique_prefix}_{n}"
    else:
        raise KeyError("One or more keys in the nested dictionary do not exist.")
    ...
    while True:
        ...
        s=COMFY_HOME+f"/output/api/{unique_prefix}_{n}"+"*"+suffix
        #print('looking for,',s)
        g=glob.glob(s)
        if g:
